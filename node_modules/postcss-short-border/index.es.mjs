...
var index = postcss.plugin('postcss-short-border', opts => {
  // options
  const prefix = 'prefix' in Object(opts) ? `-${opts.prefix}-` : '';
  const skip = 'skip' in Object(opts) ? String(opts.skip) : '*'; // property pattern

  const propertyMatch = new RegExp(`^${prefix}(border(?:-(color|style|width))?)$`); // process a matched declaration

  const processMatchedDeclaration = decl => {
    // unprefixed property
    const property = decl.prop.match(propertyMatch)[1]; // inner-property (color, style, width)

    const innerProperty = decl.prop.match(propertyMatch)[2]; // if a prefix is in use
    ...
    if (innerProperty) {
      ...
      const values = postcss.list.space(decl.value); // if the values contain a skip token
...
const properties = ['width', 'style', 'color']; // border sides
