...
var index = postcss.plugin('postcss-short-border-radius', opts => {
  const prefix = 'prefix' in Object(opts) ? `-${opts.prefix}-` : '';
  const skip = 'skip' in Object(opts) ? String(opts.skip) : '*'; // border-radius selector pattern
  ...
  return root => {
    ...
    root.walkDecls(borderRadiusPropertyRegExp, decl => {
      ...
      const properties = propertiesBySide[decl.prop.match(borderRadiusPropertyRegExp)[1]]; // spaced-separated values (top, right, bottom, left)

      const values = postcss.list.space(decl.value); // for each side property

      properties.forEach((side, index) => {
        // value of the current or first side
        const value = index in values ? values[index] : values[0]; // if the value is not a skip token
